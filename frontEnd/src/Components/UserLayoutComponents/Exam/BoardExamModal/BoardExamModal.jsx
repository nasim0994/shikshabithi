...
export default function BoardExamModal({
  boardExam,
  boardExamModal,
  setBoardExamModal,
}) {
  ...
  console.log("WOW", boardExam);

  return (
    <>
      <div className={`overlay  ${boardExamModal && "overlay_show"}`}></div>
      <div
        className={`modal w-[90%] sm:w-[380px] p-6 rounded ${
          boardExamModal && "modal_show"
        }`}
      >
        {loggedUser?.success ? (
          <div className="flex flex-col justify-center items-center text-center gap-4">
            ...
            <h2 className="text-neutral/90">
              ...
              {boardExam?.negativeMark} নম্বর কাটা হবে
            </h2>

            <div className="flex gap-4">
              <Link
                to={`/academy/board-exam/mcqExam/${boardExam?._id}`}
                className="bg-primary text-base-100 px-4 py-1.5 rounded hover:bg-primary/95 duration-150"
              >
                Start Now
              </Link>
              <button
                onClick={() => setBoardExamModal(false)}
          ...
          <LoginWarning setModal={setBoardExamModal} />
        )}
      </div>
    </>
  );
