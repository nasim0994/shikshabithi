...
export default function EditNoticesPage() {
  ...
  const handleEdit = async (e) => {
    ...
    let res = await updateNotice({ id, formData });

    if (res?.data?.success) {
      toast.success("Notice edit success");
      navigate(`/notices?avtive=${selectedCategory}`);
    } else {
  ...
  return (
    <div className="bg-base-100 shadow rounded p-6">
      <div className="text-center">
        <h3 className="text-neutral text-xl">Edit Notice</h3>
      ...
      >
        ...
        <div>
          ...
          <input type="text" name="url" defaultValue={notice?.url} />
        </div>
