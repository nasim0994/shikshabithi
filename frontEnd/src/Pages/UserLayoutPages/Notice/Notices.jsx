...
export default function Notices() {
  ...
  useEffect(() => {
    if (!active) {
      navigate("/notices?active=academy");
      setActiveCategory(1);
    ...
    <div>
      <div className="bg-base-100 p-4 rounded shadow">
        <div className="flex justify-between items-center">
          <h3 className="text-primary font-medium text-xl">Notices</h3>
          ...
          {categories?.map((category) => (
            <button
              ...
              onClick={() => {
                ...
                setSelectedSubject("");
                navigate(
                  `/notices?active=${category?.name.toLocaleLowerCase()}`
                );
              }}
