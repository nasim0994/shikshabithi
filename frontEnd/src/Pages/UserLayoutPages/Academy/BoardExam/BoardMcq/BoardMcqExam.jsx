...
import { FaBookmark, FaQuestion, FaAward, FaPercent } from "react-icons/fa";
import { IoIosTime } from "react-icons/io";
import { GrCheckboxSelected } from "react-icons/gr";
import { useSelector } from "react-redux";
import ModelTestSubmitModal from "../../../Exam/ModelTestAttend/ModelTestSubmitModal";
import ModelTestSkeleton from "../../../../../Components/Skeleton/ModelTestSkeleton";
import { useAddBoardExamResultMutation } from "../../../../../Redux/api/board/boardExamResultApi";
import { toast } from "react-toastify";

export default function BoardMcqExam() {
  let { id } = useParams();
  const navigate = useNavigate();
  const { loggedUser } = useSelector((store) => store.user);

  const { data, isLoading } = useGetSingleBoardMcqQuery(id);
  let boardExam = data?.data;

  let duration = boardExam?.duration;
  let mcqs = boardExam?.mcqs;
  let totalQuestion = boardExam?.mcqs?.length;
  let totalMark = boardExam?.mcqs?.length;
  let passMark = boardExam?.passMark;
  ...
  const [addBoardExamResult, { isLoading: addLoading }] =
  ...
  const handleExamSubmit = async () => {
    ...
    const obtainMark = totalRightAns * singleQuestionMark;
      ...
      <section>
        <div className="shadow rounded overflow-hidden bg-base-100">
          <div className="bg-primary text-base-100 p-4 flex justify-between items-center">
            <h3 className="text-lg font-medium">Board Exam</h3>
