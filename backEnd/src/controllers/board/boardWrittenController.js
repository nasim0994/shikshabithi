const Model = require("../../models/board/boardWrittenModel");
...
exports.insert = async (req, res) => {
  try {
    ...
    if (result?._id) {
      res.status(200).json({
        success: true,
        message: "Board written add success",
        data: result,
      });
    } else {
      res.status(500).json({
    ...
    res.status(400).json({ message: "Error creating Boardwritten", error });
  }
};
...
exports.get = async (req, res) => {
  ...
  try {
    ...
    const result = await Model.find(query)
      .skip(skip)
      .limit(limit)
      .populate("board class subject writtens");

    const total = await Model.countDocuments(query);
    const pages = Math.ceil(parseInt(total) / parseInt(limit));
    res.status(200).json({
      success: true,
      message: "Board written get success",
      meta: {
        total,
        pages,
        page,
...
exports.getSingle = async (req, res) => {
  try {
    const result = await Model.findById(req.params.id).populate(
      "board class subject writtens"
    );
    if (!result) {
      return res.status(404).json({ message: "Boardwritten not found" });
    }
    res.status(200).json({
      success: true,
      message: "get success",
